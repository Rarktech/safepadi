...
async function showTransactionDetail(psid: string, txnId: string) {
    try {
        ...
        if (t.status === 'AWAITING_PROOF' && role === 'buyer') {
        ...
        if (!['COMPLETED', 'CANCELLED', 'REFUNDED'].includes(t.status) && !actions.find(a => a.title === '🚩 Dispute')) {
            actions.push({ title: '🚩 Dispute', payload: `DISPUTE_TXN_${t.id}` });
        }
...
async function handlePostback(psid: string, payload: string) {
    ...
    // ── Transaction actions ───────────────────────────────────────────────────
        ...
        const disputeId = role === 'BUYER'
            ? payload.replace('DISPUTE_RETURN_BUYER_', '')
            : payload.replace('DISPUTE_RETURN_SELLER_', '');
        try {
            const p = await getProfile(psid);
            ...
            await sendMsg(psid, { text: `❌ ${err.response?.data?.error || 'Failed to confirm return. Please try again.'}` });
        }
