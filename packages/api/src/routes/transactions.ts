...
import { sendTransactionInvoiceEmail, sendNewTransactionRequestEmail, sendTransactionAcceptedEmail, sendTransactionDeclinedEmail, sendDeliverySubmittedEmail, sendTransactionCompletedEmail, sendMilestoneReleasedEmail, sendPaymentConfirmedEmail } from '../services/email';
...
router.get('/', async (req, res) => {
    try {
        const { seller_safetag, safetag, status, category, transaction_type } = req.query;
        let query = supabase.from('transactions').select('*, buyer:buyer_id(*), seller:seller_id(*), milestones:transaction_milestones(*)');
...
router.patch('/:id/status', requireUserOrBot, async (req, res) => {
    ...
    try {
        ...
        // MILESTONE transactions on Telegram/Discord have a dedicated per-phase endpoint
        // (PATCH /:id/milestones/:mId/status) — never let them fall through to this
        // whole-transaction cascade, which is only correct for the platforms with no
        // per-milestone UI (WhatsApp/Instagram/Messenger/Apple confirm the whole job at once).
        if (['confirm_receipt', 'complete_yes', 'complete_confirmed'].includes(status) && txn.transaction_type === 'MILESTONE') {
            let actorPlatform: string | undefined = (req as BotOrUserRequest).botPlatform;
            if (!actorPlatform && actorId) {
                const { data: linked } = await supabase
                    ...
                    .eq('is_primary', true)
                    .maybeSingle();
                actorPlatform = linked?.platform;
            }
            if (actorPlatform === 'telegram' || actorPlatform === 'discord') {
                return res.status(400).json({ error: 'USE_MILESTONE_ENDPOINT', message: 'This transaction has individual phases — please complete and release each phase separately from the transaction view.' });
            }
        }
        ...
        // Referral/community commissions, badges, trade-count milestones, and
        // auto-settlement payout — shared with the per-milestone finalize path.
        if (newStatus === 'FINALIZED') {
            try {
                await runFinalizeSideEffects(txn);
            } catch (e: any) {
                console.error('❌ Finalize side-effects failed:', e?.message || e);
            }
        }
...
router.patch('/:id/milestones/:mId/status', requireUserOrBot, async (req, res) => {
    try {
        ...
        if (txn.status === 'DISPUTED' || txn.status === 'RETURN_PENDING') {
            return res.status(400).json({ error: 'TRANSACTION_DISPUTED', message: "This transaction has an open dispute. Milestone actions are paused until it's resolved." });
        }
        ...
        if (status === 'COMPLETED') {
            const { count } = await supabase
                ...
                .select('id', { count: 'exact', head: true })
                .eq('milestone_id', mId);
            const hasProof = (count ?? 0) > 0 || !!milestone.proof_url;
            if (!hasProof) {
                return res.status(400).json({ error: 'PROOF_REQUIRED', message: 'Please upload proof of delivery for this phase before marking it complete.' });
            }

            const sorted = [...(txn.milestones || [])].sort((a: any, b: any) => a.index_num - b.index_num);
            const firstNonReleased = sorted.find((m: any) => m.status !== 'RELEASED');
            if (firstNonReleased && firstNonReleased.id !== mId) {
                return res.status(400).json({ error: 'OUT_OF_SEQUENCE', message: `Please complete and release "${firstNonReleased.title}" before starting this phase.` });
            }
        }

        if (status === 'RELEASED' && milestone.status !== 'COMPLETED') {
            return res.status(400).json({ error: 'NOT_YET_COMPLETED', message: 'This phase must be marked complete by the seller before it can be released.' });
        }
...
router.post('/:id/upload-proof-files', upload.array('files', 10), async (req, res) => {
    ...
    try {
        ...
        const milestoneId = (req.body as any).milestone_id || null;
        if (isMilestone && milestoneId && !(txn.milestones || []).some((m: any) => m.id === milestoneId)) {
            return res.status(400).json({ error: 'Milestone not found for this transaction' });
        }
        ...
        for (const file of files) {
            ...
            proofRecords.push({
                ...
                file_size: file.size,
                milestone_id: isMilestone ? milestoneId : null,
            });
        }
        ...
        if (isMilestone) {
            // Milestone transactions: proof belongs to one phase, not the whole project.
            // Leave the parent transaction status untouched — only the milestone's own
            // status endpoint (PATCH .../milestones/:mId/status) advances things, and it
            // already sends the correct buyer/seller notifications once the seller marks
            // the phase complete. Just record the proof link on the milestone row.
            if (milestoneId) {
                await supabase.from('transaction_milestones').update({ proof_url: proofRecords[0].file_url }).eq('id', milestoneId);
            }
            return res.json({ success: true, count: proofRecords.length });
        }
...
router.post('/:id/upload-proofs', async (req, res) => {
    const { id } = req.params;
    const { proofs, milestone_id: milestoneIdInput } = req.body;
    try {
        ...
        const isMilestone = txn.transaction_type === 'MILESTONE';
        const milestoneId = milestoneIdInput || null;
        if (isMilestone && milestoneId && !(txn.milestones || []).some((m: any) => m.id === milestoneId)) {
            return res.status(400).json({ error: 'Milestone not found for this transaction' });
        }
        ...
        if (isMilestone) {
            // Milestone transactions: don't flip the whole project's status — that's
            // owned by the milestone status endpoint, which also handles notifications.
            if (milestoneId && proofs?.length) {
                await supabase.from('transaction_milestones').update({ proof_url: proofs[0].url }).eq('id', milestoneId);
            }
...
router.post('/:id/upload-proof', async (req, res) => {
    const { id } = req.params;
    const { proof_url, milestone_id: milestoneIdInput } = req.body;
    try {
        let txn, fetchError;
        ...
        if (isMilestone) {
            // Milestone transactions: don't flip the whole project's status or fire the
            // whole-order notification — the milestone status endpoint owns that.
            if (milestoneId) {
                await supabase.from('transaction_milestones').update({ proof_url }).eq('id', milestoneId);
            }
            return res.json({ success: true });
        }
