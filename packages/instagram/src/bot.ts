...
async function checkAndGreet(psid: string) {
    ...
    trackBotEvent(`instagram:${psid}`, 'bot_registration_prompted', {});
...
async function handlePostback(psid: string, payload: string) {
    ...
    // ── Reviews ───────────────────────────────────────────────────────────────
        ...
        try {
            ...
            await sendMsg(psid, { attachment: { type: 'image', payload: { url: badgeCardUrl, is_reusable: false } } });
            await sendMsg(psid, btnTemplate(statsMsg, [{ type: 'web_url', url: reviewsUrl, title: '⭐ View My Reviews' }]));
        } catch (_) {
